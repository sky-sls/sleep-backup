...
import numpy as np
from ustaging.sequences.base_sequence import BaseSequence, requires_all_loaded
from ustaging.errors import MarginError
...
class BatchSequence(BaseSequence):
    ...
    def __init__(self,
        ...
        Args:
            ...
            augmenters:        (list)   List of ustaging.augmentation.augmenters
    ...
    def total_periods(self):
        print(f"❤❤❤ ljy： ^^^^^^^^^^^^^ self.cum_periods_per_pair={self.cum_periods_per_pair}\n"
              f"             ^^^^^^^^^^^^^ total_periods = {self.cum_periods_per_pair[-1]}")
    ...
    def total_periods_minus_margins(self):
        print(f"❤❤❤ ljy： ^^^^^^^^^^^^^ self.cum_periods_per_pair_minus_margins={self.cum_periods_per_pair_minus_margins}\n"
              f"             ^^^^^^^^^^^^^ total_periods_minus_margins = {self.cum_periods_per_pair_minus_margins[-1]}")
        """ Return the som of n_periods across all SleepStudy objects """
    ...
    def margin(self, new_margin):
        ...
        if self.all_loaded:
            ...
            print(f"❤❤❤ ljy： ^^^^^^^^^^^^^ self.periods_per_pair={self.periods_per_pair}\n"
                  f"new_margin={new_margin}\n"
                  f"minus_margin_cum_periods={minus_margin_cum_periods}")
    ...
    def __len__(self):
        ...
        if self.all_loaded:
            print(f"❤❤❤ ljy： ^^^^^^^^^^^^^ self.total_periods_minus_margins={self.total_periods_minus_margins}\n"
                  f"self.batch_size={self.batch_size}\n"
                  f"len(batch_seq)={int(np.ceil(self.total_periods_minus_margins/self.batch_size))}")
    ...
    def get_single_study_full_seq(self, study_id, reshape=False):
        """
        Return 【all periods/epochs/segments(ljy: 个数=seq_length)】 of data (X, y) of a SleepStudy.
        ...
        Returns:
            ...
            y: ndarray of labels, shape [-1, 1] （ljy注释：[-1, data_per_period, 1]？）
        """
        with self.dataset_queue.get_study_by_id(study_id) as ss:
            x, y = ss.get_all_periods()
            if reshape:
                # x is shape [-1, data_per_period, n_channels]
                # reshape to [-1, seq_length(ljy: 23/35 etc.), data_per_period(ljy: 30s * 128Hz), n_channels]
                shape = [-1] + self.batch_shape[1:]
                border = x.shape[0] % shape[1]
                if border:
                    # OBS: We remove border if needed (❤ljy: data_per_period % b_s有余数，即3840 % 16)
                    x = x[:-border]
                    y = y[:-border]
                x = x.reshape(shape)
    ...
    def single_study_seq_generator(self, study_id, margin=None,
        ...
        Args:
            ...
            overlapping:  Yield overlapping batches (sliding window). Otherwise ❤ljy：细节
        ...
        from ustaging.sequences import batch_wrapper
    ...
    def get_period(self, sleep_study, period_idx, allow_shift_at_border=True,
        ...
        nearest sequence of the same length that fits within the full sequence ❤ljy细节
        ...
        print(f"****** ❤ ljy: sleep_study.n_periods={sleep_study.n_periods}")
        try:
    ...
    def get_batch(self, batch_idx):
        ...
        # ❤ljy：np.searchsorted - https://blog.csdn.net/qq_33757398/article/details/89876088
        ...
        with self.dataset_queue.get_study_by_idx(study_idx) as sleep_study:
            ...
            local_period_start = global_period_start - previous_periods + self.margin  # ljy: +上一半segment(作为本段前半段margin)
