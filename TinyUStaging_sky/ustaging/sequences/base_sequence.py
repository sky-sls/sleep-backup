...
from ustaging.preprocessing.scaling import apply_scaling, assert_scaler
from ustaging.utils import assert_all_loaded
...
class BaseSequence(_BaseSequence):
    ...
    def get_class_counts(self):
        ...
        for im in self.dataset_queue:
            ...
            for cls, count in count_dict.items():
                if cls == self.n_classes:  # ljy改20221017: ignore_class通常为n_classes
                    continue
                counts[cls] += count
        return counts
    ...
    def augmenters(self, list_of_augs):
        """
        Initialize and set a list of ustaging.augmentation.augmenters Augmentor
        objects.
        Args:
            list_of_augs:
        Returns:
        """
        from ustaging.augmentation import augmenters
