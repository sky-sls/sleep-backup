...
import numpy as np
from ustaging.sequences import BatchSequence
...
class BalancedRandomBatchSequence(BatchSequence):
    ...
    def sample_prob(self, values):
        ...
        if values is None:
            ...
            if not isinstance(values, (list, tuple, np.ndarray)) or \
                    len(values) != self.n_classes:
                raise ValueError("'sample_prob' should be an array of "
                                "length n_classes. got {} "
                                "(type {})".format(values, type(values)))
            self._sample_prob = np.array(values, dtype=np.float32)
        ...
        while tries < max_tries:
            with self.dataset_queue.get_random_study() as sleep_study:
                try:
                    ...
                    if self.margin > 0:
                        idx += np.random.randint(-5, 6) #sky0313 随机偏移
                        # pass #sky0309 不随机偏移，保证采样类在序列中心
