...
from ustaging.callbacks.callbacks import SaveOutputAs2DImage, SavePredictionImages
from ustaging.callbacks import Validation, MemoryConsumption
from ustaging.train.utils import get_steps

#sky0306
import numpy as np
#sky0306
class CheckNaNWeights(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        logs = logs or {}
        loss_val = logs.get("loss", None)
        if loss_val is not None and (np.isnan(loss_val) or np.isinf(loss_val)):
            raise RuntimeError(f"NaN/Inf loss detected at epoch {epoch+1}: {loss_val}")

        for v in self.model.weights:
            arr = v.numpy()
            if np.isnan(arr).any() or np.isinf(arr).any():
                print(f"[NaN DETECTED] epoch={epoch+1}, weight={v.name}")
                raise RuntimeError(f"NaN/Inf found in weight: {v.name}")
...
def ignore_class_wrapper(loss_func, n_pred_classes, logger):
    ...
    def wrapper(true, pred):
        ...
        # logger(f"-----true.shape={true.shape}, pred.shape={pred.shape}")
        # ❤ljy: tf.where(condition, x（True则输出）, y(False则输出))
...
class Trainer(object):
    ...
    # ljy注释：keywords见hparams.yaml
    def compile_model(self, optimizer, loss, metrics, reduction,
                      ignore_class_int=None, check_sparse=False, optimizer_kwargs={}, loss_kwargs={}, **kwargs):
        """
        ...
        # model.summary(): 20211028
        self.logger("{} Model Summary2\n"
                    "-------------------".format(__class__.__name__))
        self.logger(self.model.summary(line_length=100))

        self.logger("-------------------------------------------------")
        self.logger("*********** get FLOPs ********** ")
        # Calculae FLOPS
        flops = get_flops(self.model, batch_size=16)
    ...
    def _fit(self,
        ...
        if val is None:
            ...
            callbacks = [validation, MeanReduceLogArrays()] + callbacks  # LJY改17-20210824
        
        
        #sky0306
        callbacks.append(CheckNaNWeights())
        ...
        ########## ljy加 - 20211017
        # callbacks.append(SaveOutputAs2DImage(train, val, self.model, out_dir='ljy_imgs', logger=self.logger))
        # callbacks.append(SavePredictionImages(train, val, outdir='ljy_imgs'))
        #############
