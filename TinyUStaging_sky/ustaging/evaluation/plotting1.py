...
def get_hypnogram(y_pred, y_true=None, id_=None):
    def format_ax(ax):
        ...
        ax.set_yticks([0,4,1,2,3,5]) # ljy改16
    ...
    fig.suptitle("Hypnogram {}".format(id_ or "???"))  # ljy改15:缩短标题

    # Plot predicted hypnogram
    ax1.step(ids+1, y_pred, color="red", label="Pred")
    format_ax(ax1)
    if y_true is not None:
        ax2.step(ids+1, y_true, color="blue", label="True")
        format_ax(ax2)
...
def plot_confusion_matrix(y_true, y_pred, n_classes,
    ...
    print("unique_labels(y_true, y_pred): ", unique_labels(y_true, y_pred))
