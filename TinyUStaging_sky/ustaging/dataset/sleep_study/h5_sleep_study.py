...
import numpy as np
from ustaging import errors, Defaults
from ustaging.dataset.sleep_study.abc_sleep_study import AbstractBaseSleepStudy
from ustaging.io.channels import ChannelMontageTuple
from ustaging.io.high_level_file_loaders import get_org_include_exclude_channel_montages
...
class H5SleepStudy(AbstractBaseSleepStudy):
    ...
    def _load_with_any_in(self, channel_sets, channels_in_file):
        ...
        for i, channel_set in enumerate(channel_sets):
            try:
                ...
                # org_channels, include_channels, _ = \ # 20210721改↓
    ...
    def get_class_counts(self, as_dict=False):
        ...
        #print(classes)
        labels = self.get_full_hypnogram().ravel()
        #print(labels[:10])
        ...
        for label in labels:
            if label == 5: continue
            counts[label] += 1
        # counts = np.array([len(labels(labels == c)) for c in classes])
        if as_dict:
            return  {cls: count for cls, count in zip(counts.keys(), counts.values())} # {cls: count for cls, count in zip(classes, counts)}
        else:
            return counts
