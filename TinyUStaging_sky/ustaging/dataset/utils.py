...
These functions are used in particular by the ustaging.dataset SleepStudy and
...
def unpack_file_list(file_list_path):
    ...
    LIST_OF_FILES.txt file (created by the ustaging.bin.cv_split script in some
    circumstances).
...
def find_subject_folders(data_dir, folder_regex=None):
    ...
    ustaging.bin.cv_split script in some circumstances, and stores paths to
    subject_dirs.
