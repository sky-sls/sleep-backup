...
class Validation(Callback):
    ...
    def _compute_counts(self, pred, true, ignore_class=None):
        ...
        true = true.ravel()
        # print(f"[_compute_counts]------pred.shape={pred.shape},true.shape={true.shape}")
        ...
        for id_, sequence in zip(self.IDs, self.sequences):
            ...
            for i, sleep_study_context in enumerate(study_iterator):
                ...
                tps, rel, sel = self._compute_counts(pred=pred_numpy,
                                                     true=y,
                                                     ignore_class=self.n_classes) # ljy改20221017：'其他'.映射值=n_classes
                true_pos[id_] += tps
                relevant[id_] += rel
                selected[id_] += sel

                                # =====sky0310 打印当前 subject 的真实/预测类别分布 =====
                self.logger(f"[DEBUG][{id_}][study {i+1}/{n_val}] true counts: {rel.tolist()}")
                self.logger(f"[DEBUG][{id_}][study {i+1}/{n_val}] pred counts: {sel.tolist()}")
