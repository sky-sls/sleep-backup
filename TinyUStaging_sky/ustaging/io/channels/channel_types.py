...
from ustaging.io.channels import ChannelMontageTuple, ChannelMontage
...
EOG_REGEX = re.compile(r"(\bEEG|\b{})".format(r"|\b".join(EEG_CHANNELS)), re.IGNORECASE) # ljy改20221017：直接将EOG替换为EEG
    ...
    for channel in channels:
        ...
        if n_matches > 1:
            ...
            if eeg_and_mastoid and is_eeg(channel.channel.replace("EEG", "")):
                ...
                # match = "AMBIGUOUS"
                match = "EEG"  # ljy改：20221027，默认都是EEG
        elif n_matches == 0:
            # Something else
            match = "OTHER"
        else:
