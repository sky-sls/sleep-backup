...
from ustaging.bin.evaluate import (set_gpu_vis,
                                   get_and_load_one_shot_model,
                                   get_logger, plot_hypnogram_PAD)
from ustaging.hypnogram.utils import dense_to_sparse
from ustaging.io.channels import infer_channel_types, VALID_CHANNEL_TYPES
from ustaging.io.channels import auto_infer_referencing as infer_channel_refs
from ustaging import Defaults
from pprint import pformat
from collections import namedtuple

# ljy改 - 20221026，PAD上是2EEG
def get_argparser():
    """
    Returns an argument parser for this script
    """
    parser = ArgumentParser(description='Predict using a TinyUStaging model.')
    ...
    parser.add_argument("--channels", nargs='+', type=str, default=None, # "EEG1++EEG2"
                        required=True,
    ...
    parser.add_argument("--project_dir", type=str, default=None,
                        help='Path to TinyUStaging project folder. Cannot specify both --model and --project_dir. '
                             'If neither are specified, --project_dir defaults to the current working directory.')
    parser.add_argument("--data_per_prediction", type=int, default=None, # 训练时为3840，PAD预测时为256*30
    ...
    parser.add_argument("--plot_hypnograms_PAD", action="store_true",
                        help="Add single plot with the predicted labels only"
                             " to folder [out_dir]/plots/hypnograms.")  # LJY加 - 20221026
    return parser
...
def get_processed_args(args):
    ...
    from ustaging.utils.scriptutils.scriptutils import assert_project_folder
...
def predict_study(study, model, channel_groups, no_argmax, logger=print):
    ...
    if callable(getattr(pred, "numpy", None)):
        pred = pred.numpy()
    if isinstance(pred, np.ndarray):  ## ljy改3：pred.numpy()报错
...
def save_hyp(path, pred, **kwargs):
    ...
    stage_strings = np.vectorize(Defaults.get_class_int_to_stage_string_PAD().get)(pred.ravel())
...
def save_ids(path, pred, period_length_sec, **kwargs):
    ...
    with open(path, "w") as out_f:
        print(f"ljy debug [save_ids] : len(ids) = {len(ids[0])}\n")
        print(f"i.shape = {(ids[0].shape)}, d.shape = {(ids[1].shape)}, s.shape = {(ids[2].shape)}")
        for i in range(len(ids[0])):
            i, d, s = ids[0][i], ids[1][i], ids[2][i]
        # for i, d, s in ids:
            out_f.write(f"{i},{d},{s}\n")
...
def get_sleep_study(psg_path,
    ...
    from ustaging.dataset.sleep_study import SleepStudy
    dir_, regex = os.path.split(os.path.abspath(psg_path))
    # 如dir_:  /export/Nanjing/ljy/demo/datasets/pad, regex:  liyang.h5
    print("ljy-log-20221027:", "\ndir_: ", dir_, "\nregex: ", regex)
...
def run(args, return_prediction=False, dump_args=None):
    ...
    warnings_file_name = os.path.splitext(log_file_name)[0] + ".warnings" # ljy未改9
    ...
    # 画预测图
    subjectName = os.path.splitext(os.path.split(args.f)[-1])[0]
    if args.plot_hypnograms_PAD:
        plot_hypnogram_PAD(args.project_dir, pred, id_=subjectName, true=None)
