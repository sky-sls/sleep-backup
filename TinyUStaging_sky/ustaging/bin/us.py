...
def get_parser():
    from ustaging import bin, __version__
    import pkgutil
    mods = pkgutil.iter_modules(bin.__path__)

    ids = "TinyUStaging ({})".format(__version__)
    sep = "-" * len(ids)
    usage = ("sky [--help] script [script args]... [--seed]\n\n"
             "%s\n%s\n"
             "Available scripts:\n") % (ids, sep)
...
def entry_func():
    ...
    if parsed.seed is not None:
        ustaging.Defaults.set_global_seed(parsed.seed)

    # Import the script
    import importlib
    mod = importlib.import_module("ustaging.bin." + script)
