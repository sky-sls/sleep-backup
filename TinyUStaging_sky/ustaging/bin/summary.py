...
def get_argparser():
    ...
    parser.add_argument("--dataset_name", type=str, default='sedf_sc',
                        help="Dataset name to save summary_{dataset}.csv")
    return parser


def print_reduced_mean(df, print_all=False, round_=4, out_dir=None, dataset_name='sedf_sc'):
    """
    ...
    if out_dir is not None:
        if dataset_name is not None:
            df.to_csv(os.path.join(out_dir, f'summary_{dataset_name}.csv'))
        else:
...
def parse_and_add(file_, results, drop_rows, drop_cols, out_dir=None, dataset_name='sedf_sc'):
    """
    ...
    if out_dir is not None:
        oo = o.copy()
        oo['mean'] = o.mean(axis=1)
        if dataset_name is not None:
            oo.to_csv(os.path.join(out_dir, f"summary_{dataset_name}_ALL.csv"))
        else:
            oo.to_csv(os.path.join(out_dir, "summary_ALL.csv"))
    return o


def parse_results(csv_files, drop_rows, drop_cols, print_all, round_, out_dir=None, dataset_name='sedf_sc'):
    """
