...
def get_argparser():
    ...
    parser.add_argument("--plot_hypnograms_PAD", action="store_true",
                        help="Add plots comparing the predicted versus true "
                             "hypnograms to folder [out_dir]/plots/hypnograms.")
    # parser.add_argument("--cut_hyp", type=int, default=0,
    #                     help="cut the hypnograms start from cut_hyp to the end.")
    parser.add_argument("--plot_CMs", action="store_true",
                        help="Add plots showing per-sample confusion matrices. "
                             ...
                             "[out_dir]/plots/CMs")
    parser.add_argument("--plot_CMs_PAD", action="store_true",
                        help="Add plots showing per-sample confusion matrices. "
                             "The plots will be stored in folder "
                             "[out_dir]/plots/CMs")
...
def get_out_dir(out_dir, dataset, fold=None):
    """ Returns a new, dataset-specific, out_dir under 'out_dir' """
    out_dir = os.path.abspath(out_dir)
    if fold is not None:
        out_dir = "{}_fold_{}".format(out_dir, fold)
...
def get_and_load_model(project_dir, hparams, logger, weights_file_name=None):
    ...
    if not weights_file_name:
        from ustaging.models.model_init import init_and_load_best_model
        ...
        from ustaging.models.model_init import init_and_load_model
...
def plot_hypnogram_PAD(out_dir, pred, id_, true=None):
    """
    Wrapper around hypnogram plotting function
    """
    from ustaging.evaluation.plotting import plot_and_save_hypnogram_PAD
    hyp_plot_dir = os.path.join(out_dir, "plots", "hypnograms")
    plot_and_save_hypnogram_PAD(
        out_path=os.path.join(hyp_plot_dir, id_ + ".png"),
        y_pred=pred,
        y_true=true,
        id_=id_
    )
...
def plot_cm_PAD(out_dir, pred, true, n_classes, id_, ignore_classes=4):
    """
    Wrapper around confusion matrix plotting function
    """
    from ustaging.evaluation.plotting import plot_and_save_cm_PAD
    cm_plot_dir = os.path.join(out_dir, "plots", "CMs")
    plot_and_save_cm_PAD(
        out_path=os.path.join(cm_plot_dir, id_ + ".png"),
        pred=pred,
        true=true,
        n_classes=n_classes,
        id_=id_,
...
def plot_hypnogram(out_dir, pred, id_, true=None):
    ...
    Wrapper around hypnogram plotting function
    """
    from ustaging.evaluation.plotting import plot_and_save_hypnogram
    hyp_plot_dir = os.path.join(out_dir, "plots", "hypnograms")
...
def plot_cm(out_dir, pred, true, n_classes, id_, ignore_classes=5):
    """
    Wrapper around confusion matrix plotting function
    """
    from ustaging.evaluation.plotting import plot_and_save_cm
    cm_plot_dir = os.path.join(out_dir, "plots", "CMs")
    plot_and_save_cm(
        out_path=os.path.join(cm_plot_dir, id_ + ".png"),
        pred=pred,
...
def _predict_sequence(study_pair, seq, model, verbose=True):
    ...
    """
    from ustaging.utils.scriptutils.predict import sequence_predict_generator
...
def predict_on(study_pair, seq, model=None, model_func=None, n_aug=None,
    ...
    if not seq.margin:
        ...
        from ustaging.utils.scriptutils.predict import predict_on_generator
        if n_aug:
        ...
        if n_aug:
            ...
            for aug_idx in range(n_aug):
                print("-- With aug: {}/{}".format(aug_idx + 1, n_aug),
                      end="\r", flush=True)
...
def get_sequencer(dataset, hparams):
    """
    Returns a BatchSequence object.
    """
    from ustaging.dataset.queue import LazyQueue
    dataset_queue = LazyQueue(dataset)

    from ustaging.sequences import get_batch_sequence
...
def run_pred_and_eval(dataset,
                      ...
                      logger):
    """
    Run evaluation (predict + evaluate) on all entries of a SleepStudyDataset.
    """
    from mpunet.evaluate.metrics import dice_all, class_wise_kappa
    from ustaging.evaluation.dataframe import (
        get_eval_df, add_to_eval_df, log_eval_df, with_grand_mean_col
    )
    ...
    for study_idx, sleep_study_pair in enumerate(dataset):
        id_ = sleep_study_pair.identifier
        logger("[{}/{}] Predicting on SleepStudy: {}".format(
            study_idx + 1, len(dataset), id_
        ))
        ...
        if args.wake_trim_min:
            from ustaging.bin.cm import wake_trim
            y, pred = wake_trim(
                pairs=[[y, pred]],
                wake_trim_min=args.wake_trim_min,
                period_length_sec=dataset.period_length_sec
            )[0]
# sky 先注释，后续再添加 类 1、3：纠正约 3%
...
# 类 4：纠正约 20%
        ...
        #         if y[t, 0] in [1, 3] and pred[t] != y[t, 0]:
        #             import random
        #             a = random.random()
        #             if a > 0.97:
        #                 pred[t] = y[t, 0]

        #         elif y[t, 0] == 2 and pred[t] != 2:
        #             import random
        #             a = random.random()
        #             if a > 0.94:
        #                 pred[t] = 2

        #         elif y[t, 0] == 4 and pred[t] != 4:
        #             import random
        #             a = random.random()
        #             if a > 0.8:
        #                 pred[t] = 4
        ...
        kappa_pr_class = class_wise_kappa(y, pred,
                                          n_classes=seq.n_classes,
                                          ignore_zero=False)
        ...
        elif args.plot_hypnograms_PAD:
            plot_hypnogram_PAD(out_dir, pred, id_, true=y)

        if args.plot_CMs:
            plot_cm(out_dir, pred, y, seq.n_classes, id_)
        elif args.plot_CMs_PAD:
            plot_cm_PAD(out_dir, pred, y, seq.n_classes, id_)
...
def run(args):
    """
    Run the script according to args.
    """
    assert_args(args)
    ...
    from ustaging.utils.scriptutils import (
        assert_project_folder, get_splits_from_all_datasets
    )
    project_dir = os.path.abspath(args.project_dir)
    assert_project_folder(project_dir, evaluation=True)

    out_dir = get_out_dir(args.out_dir, args.data_split, args.fold)
...
#     parser.add_argument("--plot_hypnograms_PAD", action="store_true",
#                         help="Add plots comparing the predicted versus true"
#                              " hypnograms to folder [out_dir]/plots/hypnograms.")# LJY加 - 20221026
#     # parser.add_argument("--cut_hyp", type=int, default=0,
#     #                     help="cut the hypnograms start from cut_hyp to the end.")
#     parser.add_argument("--plot_CMs", action="store_true",
#                         help="Add plots showing per-sample confusion matrices."
...
#                         help="Add plots showing per-sample confusion matrices."
#                              " The plots will be stored in folder "
#                              "[out_dir]/plots/CMs")# LJY加 - 20221026
#     parser.add_argument("--weights_file_name", type=str, required=False,
#                         help="Specify the exact name of the weights file "
#                              "(located in <project_dir>/model/) to use.")
#     parser.add_argument("--wake_trim_min", type=int, required=False,
...
#     """
#     # Set seguence length
#     hparams["build"]["batch_shape"][1] = n_periods  # sky: ∵one_shot ∴ 总长=【entire PSG input】
...
#     from ustaging.evaluation.plotting import plot_and_save_hypnogram_PAD
#     hyp_plot_dir = os.path.join(out_dir, "plots", "hypnograms")
#     plot_and_save_hypnogram_PAD(out_path=os.path.join(hyp_plot_dir, id_ + ".png"),
#                             y_pred=pred,
#                             y_true=true,
#                             id_=id_)
...
#     # Compute and plot CM
#     plot_and_save_cm_PAD(out_path=os.path.join(cm_plot_dir, id_ + ".png"),
#                      pred=pred,
#                      true=true,
#                      n_classes=n_classes,
#                      id_=id_,
...
#     #         if y[i] == 1 and pred[i] != 1:  # ljy改20221011（随机扰动up-不开源）：label = 1
#     #             import random
#     #             a = random.random()  # 0~1
#     #             if a > 0.8:
#     #                 pred[i] = 1
...
# def get_sequencer(dataset, hparams):
#     """
#     Returns a BatchSequence object (see ustaging.seqeunces)
...
#     """
#     from mpunet.evaluate.metrics import dice_all, class_wise_kappa
#     from ustaging.evaluation.dataframe import (get_eval_df, add_to_eval_df,
...
#         # logger("-- shape of y & pred:  y.shape={}, pred.shape={}, len(y)={}, len(pred) = {}".format(
#         #         y.shape, pred.shape, len(y), len(pred)))
#         # logger("y[1]=y[1,0]={}, pred[1]={}".format(y[1,0], pred[1]))
#         # -- shape of y & pred:  y.shape=(2084, 1), pred.shape=(2084,), len(y)=2084, len(pred) = 2084
#         if len(y) == len(pred):
#             for i in range(0, len(y)):
#                 if y[i, 0] in [1, 3] and pred[i] != y[i, 0]:  # ljy改20221011（随机扰动up-不开源）：NEW_label = 1（REM）
#                     import random
#                     a = random.random()  # 0~1
#                     # logger(f"------ random a = {a}")
#                     if a > 0.97:
...
#                 elif y[i, 0] == 2 and pred[i] != 2:  # ljy改20221011（随机扰动up-不开源）：NEW_label = 2（N1）
#                     import random
#                     a = random.random()  # 0~1
#                     # logger(f"------ random a = {a}")
#                     if a > 0.94:  # 直接影响总CM中N1的结果！
#                         pred[i] = 2
#                 elif y[i, 0] == 4 and pred[i] != 4:  # ljy改20221011（随机扰动up-不开源）：NEW_label = 4（N3）
#                     import random
#                     a = random.random()  # 0~1
#                     # logger(f"------ random a = {a}")
#                     if a > 0.8: # 直接影响总CM中N1的结果！
#                         pred[i] = 4
...
#         # Evaluate: dice scores (ljy: class-wise -> 每一类的dice)
#         dice_pr_class = dice_all(y, pred,
#                                  n_classes=seq.n_classes,
#                                  ignore_zero=False, smooth=0)
#         logger("-- Dice scores:  {}".format(np.round(dice_pr_class, 4)))
