...
from ustaging.bin.evaluate import plot_cm
...
def run(args):
    ...
    if args.out_dir:
        ...
        if args.normalized:
            cm.to_csv(os.path.join(args.out_dir, f'cm_{args.CM_describe}_normalized.csv'))
        else:
            cm.to_csv(os.path.join(args.out_dir, f'cm_{args.CM_describe}.csv'))
        plot_cm(os.path.join(args.out_dir, (args.CM_describe).split()[0]), pred, true, classes, id_=args.CM_describe, ignore_classes=5)
    ...
    if args.out_dir:
        if args.CM_describe is not None:
            metrics.to_csv(os.path.join(args.out_dir, f'metrics_{args.CM_describe}.csv'))
        else:
            metrics.to_csv(os.path.join(args.out_dir, 'metrics_all.csv'))
