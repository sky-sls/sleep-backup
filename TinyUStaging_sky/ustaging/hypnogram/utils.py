...
ustaging operates with the following notation on hypnogram encodings:
...
import numpy as np
from ustaging.hypnogram import SparseHypnogram
from ustaging.hypnogram.formats import StartDurationStageFormat
from ustaging.hypnogram.stage_mapper import create_variable_ann_to_class_int_dict
...
def sparse_to_csv_file(inits, durs, stages, out_path, stage_map=None):
    ...
    (see ustaging.hypnogram.formats) separated into the 3 sub-lists and outputs
...
def dense_to_sparse(array, period_length_sec, allow_trim=False):
    ...
    'inits', 'durations' and 'stages' (see ustaging.hypnogram.formats).
...
def signal_dense_to_sparse(array, sample_rate, period_length_sec, allow_trim=False):
    ...
    (see ustaging.hypnogram.formats).
...
def ndarray_to_ids_format(array, period_length_sec, sample_rate):
    ...
    (see ustaging.hypnogram.utils for a description).
...
def sparse_hypnogram_from_ids_format(ids_tuple, period_length_sec, ann_to_class):
    ...
    Args:
        ids_tuple:          3-tuple of equal length lists of starts, durations
                            and sleep stages (see ustaging.hypnogram)
        period_length_sec:  Sleep 'epoch'/period length in seconds.
...
def load_events_file(events_file_path):
    ...
    """
    from ustaging.io.hypnogram.hyp_extractors import extract_from_start_dur_stage
