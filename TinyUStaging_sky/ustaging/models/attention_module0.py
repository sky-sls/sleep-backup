...
def attach_attention_module(net, attention_module, ratio=4, data_per_prediction=3840, kernel_size=7, logger=None):
    if logger is None:
        logger = ScreenLogger()
    ...
    if attention_module in ['se_block', 'SE', 'se']:  # SE_block
        ...
        net = cbam_block(net, ratio, data_per_prediction, kernel_size=kernel_size, logger=logger)
    else:
...
def spatial_attention(input_feature, kernel_size=7, data_per_prediction=3840, logger=None):
    ...
    logger(f'ljy---spatial_attention -- input_feature: {ori_shape} -> (TODO:) {to_shape}')
    #
