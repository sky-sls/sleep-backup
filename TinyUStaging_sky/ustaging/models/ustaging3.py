...
class UStaging3(Model):
    ...
    def __init__(self,
        ...
        if isinstance(ratio_se, (list, tuple)):
            if len(ratio_se) != 3:
                self.ratio_se = [ratio_se[0]] * 3
            else:
                self.ratio_se = ratio_se
        else:
            self.ratio_se = [ratio_se] * 3

        if isinstance(ratio_cbam, (list, tuple)):
            if len(ratio_cbam) != 3:
                self.ratio_cbam = [ratio_cbam[0]] * 3
            else:
                self.ratio_cbam = ratio_cbam
        else:
            self.ratio_cbam = [ratio_cbam] * 3
    ...
    def create_dense_modeling(self,
        ...
        if attention_module_dense is not None:
            self.logger(f'Attention (2) - Dense (after 1*1 Conv): {attention_module_dense}')
    ...
    def create_seq_modeling(self,
        ...
        if attention_module_seq is not None:
            self.logger(f'Attention (3) - Seq: {attention_module_seq}')
