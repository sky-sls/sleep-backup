from tensorflow.keras.layers import GlobalAveragePooling2D, GlobalMaxPooling2D, Reshape, Dense, multiply, Permute, \
    Concatenate, Conv2D, Add, Activation, Lambda, Dropout
...
def attach_attention_module(net, attention_module,
                            ratio_se=8,
                            ratio_cbam=8, data_per_prediction=3840, kernel_size=7,
                            logger=None):
    if logger is None:
        logger = ScreenLogger()
    ...
    if attention_module in ['se_block', 'SE', 'se']:  # SE_block
        ...
        net = cbam_block(net, ratio_cbam, data_per_prediction, kernel_size=kernel_size, logger=logger)
    else:
...
def cbam_block(cbam_feature, ratio=4, data_per_prediction=3840, kernel_size=7, logger=None):
    """Contains the implementation of Convolutional Block Attention Module(CBAM) block.
    As described in https://arxiv.org/abs/1807.06521.
    """

    cbam_feature = channel_attention(cbam_feature, ratio)  # TensorShape([8, 31*3840, 1, 22])
    cbam_feature = spatial_attention(cbam_feature, data_per_prediction, kernel_size, logger=logger)
...
def spatial_attention(input_feature, kernel_size=7, data_per_prediction=3840, logger=None):
    ...
    avg_pool = tf.reduce_max(cbam_feature, axis=3, keepdims=True)
    ...
    cbam_feature = Conv2D(filters=1,   # point-wise Conv
