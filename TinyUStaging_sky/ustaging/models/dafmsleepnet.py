...
Domain Adversarial Module Added for Cross-Subject Generalization
"""
import numpy as np
...
from tensorflow.keras.layers import Input, BatchNormalization, Cropping2D, \
                                    ...
                                    AveragePooling2D, DepthwiseConv2D, Dropout, Flatten
...
class GradientReversalLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, training=None):
        ...
        def grad_reverse(x):
            def grad(dy):
                return -self.lambda_val * dy
            return x, grad
        return grad_reverse(x)
    
    def get_config(self):
        config = super(GradientReversalLayer, self).get_config()
        config.update({'lambda_val': self.lambda_val.numpy()})
        return config


class DafmSleepNet(Model):
    """
    # ljy❤：attention_module_seq=None,  # ❤ us3新增
    # attention_module_encoder='CBAM' # ❤ us4新增 (浅层位置信息)
    ...
    def __init__(self,
                 ...
                 batch_shape,
                 depth=4,
                 dilation=((1,2), (5,1), (2,5), (1,2)),  # TODO: ASPP + 避免gridding effect
                 activation="elu",
                 ...
                 attention_module_encoder=None,
                 isChanAttn=True,
                 isSeparable=False,
                 attention_module_encoder_layer=None,
                 attention_module_bottom=None,
                 attention_module_dense=None,
                 attention_module_seq=None,  # ❤ us3新增
                 ratio_se=None,
                 ratio_cbam=None,
                 isAdd=False,
                 use_domain_adversarial=False,  # 新增：是否使用域对抗模块
                 n_domains=10,  # 新增：域的数量（受试者数量）
                 lambda_val=1.0,  # 新增：梯度反转层的lambda系数
                 domain_hidden_units=256,  # 新增：域分类器隐藏层单元数
                 logger=None,
                 build=True,
                 **kwargs):
        """
        ...
        # if data_per_prediction and batch_shape[2] < data_per_prediction:
        #     self.input_dims = data_per_prediction  # pad中需要30s*256=7680 > 3840, 直接将其改为7680！！！
        # else:
        self.input_dims = batch_shape[2] # 3840, yaml中batch需要灵活调整！
        ...
        # ljy -- attention_module
        if not attention_module_encoder or not attention_module_encoder_layer: # us4新增
            self.attention_module_encoder = None
            self.attention_module_encoder_layer = None
            self.isChanAttn = True
            self.isSeparable = False
        else:
            self.attention_module_encoder=attention_module_encoder  # us4新增
            self.attention_module_encoder_layer = int(attention_module_encoder_layer)
            self.isChanAttn = isChanAttn
        ...
        self.attention_module_seq = attention_module_seq
        self.isAdd = isAdd
        ...
        self.domain_hidden_units = domain_hidden_units

        n_attention_module = 4
        if isinstance(ratio_se, (list, tuple)):
            if len(ratio_se) != n_attention_module:
                self.ratio_se = [ratio_se[0]] * n_attention_module
            else:
                self.ratio_se = ratio_se
        else:
            self.ratio_se = [ratio_se] * n_attention_module

        if isinstance(ratio_cbam, (list, tuple)):
            if len(ratio_cbam) != n_attention_module:
                self.ratio_cbam = [ratio_cbam[0]] * n_attention_module
            else:
                self.ratio_cbam = ratio_cbam
        else:
            self.ratio_cbam = [ratio_cbam] * n_attention_module

        self.logger(f"************ sky --- ratio_se:{ratio_se} ************ ")
        self.logger(f"************ sky --- ratio_cbam:{ratio_cbam} ************ ")
        ...
        self.data_per_prediction = data_per_prediction or self.input_dims  # 放到了上面
    ...
    def create_encoder(self,
                       ...
                       regularizer=None,
                       attention_module_encoder=None,
                       attention_module_encoder_layer=None,  # 1 ->  (None, 11904, 1, 22)
                       isChanAttn=True,
                       name="encoder",
                       name_prefix=""):
        ...
        for i in range(depth):
            ...
            if attention_module_encoder_layer == i:  # 推荐为3（与pool_size=10相联合，抵消积极下采样带来的空间信息丢失）
                # attention_module （At bottom）
                self.logger("sky--- [Encoder(Downsample)] - in_.get_shape().as_list(): ",
                            in_.get_shape().as_list())  # [None, 23/31*3840, 1, 7]

                if attention_module_encoder is not None:
                    self.logger(f'Attention (1) - Encoder: {attention_module_encoder}')
                    ...
                    in_ = attach_attention_module(in_,
                                                  attention_module_encoder,
                                                  ratio_cbam=ratio_cbam,
                                                  ratio_se=ratio_se,
                                                  data_per_prediction=self.data_per_prediction,
                                                  kernel_size=5,
                                                  isChanAttn=isChanAttn,
                                                  isSeparable=True,  # 减小计算量
                                                  isAdd=self.isAdd,
                                                  logger=self.logger)
    ...
    def create_upsample(self,
        ...
        # attention_module （At bottom）
        self.logger("sky--- [Decoder(Upsample)] - cls.get_shape().as_list(): ",
                    in_.get_shape().as_list())  # [None, 23/31*3840, 1, 7]


        if attention_module_bottom is not None:
            self.logger(f'Attention (2) - Bottom: {attention_module_bottom}')  # 推荐SE
        ...
        if self.attention_module_encoder_layer == 4:  # 其实3已经是底部，3(CBAM)->SE / SE-> 4(CBAM)
            # attention_module （At bottom）
            self.logger("sky --- [Encoder(Downsample)] - in_.get_shape().as_list(): ",
                        in_.get_shape().as_list())  # [None, 23/31*3840, 1, 7]

            if self.attention_module_encoder is not None:
                self.logger(f'Attention (2-2) - Bottom_2: {self.attention_module_encoder}')
                ...
                in_ = attach_attention_module(in_,
                                              self.attention_module_encoder,
                                              ratio_cbam=ratio_cbam,
                                              ratio_se=ratio_se,
                                              data_per_prediction=self.data_per_prediction,
                                              kernel_size=5,
                                              isChanAttn=self.isChanAttn,
                                              isSeparable=self.isSeparable,  # CBAM-减小计算量
                                              isAdd=self.isAdd,
                                              logger=self.logger)
    ...
    def create_dense_modeling(self,
                              ...
                              attention_module_dense=None,
                              **kwargs):
        self.logger("sky--- [create_dense_modeling(1*1 Conv)] - in_.get_shape().as_list(): ", in_.get_shape().as_list())    # [None, 23/31*3840, 1, 【22】]

        # US4 :  attention_module - VS UStaging v2/3 -- 恢复参数 [None, 23/31*3840, 1, 【22】-> 选择'SE' 关注通道注意力]
        if attention_module_dense is not None:
            self.logger(f'Attention (3) - Dense (after 1*1 Conv): {attention_module_dense}')  # 推荐SE（依旧是'深层语义信息'占优势）

            if isinstance(self.ratio_se, list):
                ratio_se = self.ratio_se[2]
            else:
                ratio_se = self.ratio_se

            if isinstance(self.ratio_cbam, list):
                ratio_cbam = self.ratio_cbam[2]
            else:
                ratio_cbam = self.ratio_cbam
            ...
            in_ = attach_attention_module(in_,
                                          ...
                                          ratio_se=ratio_se,
                                          data_per_prediction=self.data_per_prediction,
                                          kernel_size=5,  # VS US1: 减小卷积核参数
                                          # isSeparable=False,  # 此处特征图[8, 31*3840, 1, 2], Conv_filters=1 -> 已经尽量轻量了
                                          logger=self.logger)
        ...
        self.logger("sky--- [dense(1*1 Conv)] - cls.get_shape().as_list(): ",
        ...
        self.logger("sky--- [create_dense_modeling(1*1 Conv) (cropped)] - ❤ out.get_shape().as_list(): ", out.get_shape().as_list())    # [None, 23/31*3840, 1, 7]
        self.logger("sky--- [create_dense_modeling(1*1 Conv)] - in_reshaped.get_shape().as_list(): ", in_reshaped.get_shape().as_list())  # [None, 23/31*3840, 1, 2]
        return out  # [-1, 31/23*3840, 1, 7]

    def create_domain_classifier(self, in_, regularizer=None, name_prefix=""):
        """
        创建域分类器，由两层全连接网络组成
        输入：瓶颈层的高层语义特征
        输出：样本所属域的概率分布
        """
        # 全局平均池化，将空间维度压缩
        x = AveragePooling2D((in_.shape[1], in_.shape[2]), 
                             name="{}domain_global_pool".format(name_prefix))(in_)
        
        # 展平
        x = Flatten(name="{}domain_flatten".format(name_prefix))(x)
        
        # 第一层全连接
        x = Dense(self.domain_hidden_units,
                  activation='relu',
                  kernel_regularizer=regularizer,
                  bias_regularizer=regularizer,
                  name="{}domain_dense1".format(name_prefix))(x)
        
        # 第二层全连接（输出层）
        domain_output = Dense(self.n_domains,
                              activation='softmax',
                              kernel_regularizer=regularizer,
                              bias_regularizer=regularizer,
                              name="{}domain_output".format(name_prefix))(x)
        
        return domain_output

    # @staticmethod  # ljy改
    def create_seq_modeling(self,
                            in_,  # ljy❤: [-1, 31/23*3840, 1, 7] -> 每类一个FeatureMap❤❤❤
        ...
        if attention_module_seq is not None:
            self.logger(f'Attention (4) - Seq: {attention_module_seq}')
            self.logger("sky--- [Seq (after Avg Pooling)] - cls.get_shape().as_list(): ",
                        cls.get_shape().as_list())  # [None, 23/31*3840, 1, 7]

            if isinstance(self.ratio_se, list):
                ratio_se=self.ratio_se[3]
            else:
                ratio_se = self.ratio_se

            if isinstance(self.ratio_cbam, list):
                ratio_cbam=self.ratio_cbam[3]
            else:
                ratio_cbam = self.ratio_cbam

            cls = attach_attention_module(cls,
                                          attention_module_seq,
                                          ratio_cbam=ratio_cbam,
                                          ratio_se=ratio_se,
                                          data_per_prediction=self.data_per_prediction,
                                          kernel_size=3,  # FeatureMap：[-1, 【n_periods(28/31), 1】, 7]
                                          logger=self.logger)
    ...
    def init_model(self, inputs=None, name_prefix=""):
        ...
        settings = {
            ...
            "activation": activation,
            "dilation": self.dilation,  # TODO: [[1,2], [3/5,1], [2,3/5], [1,2]...] -> 避免gridding effect
        ...
        enc, residual_cons, filters = self.create_encoder(in_=in_reshaped,
                                                          attention_module_encoder=self.attention_module_encoder,
                                                          attention_module_encoder_layer=self.attention_module_encoder_layer,
                                                          isChanAttn=self.isChanAttn,
                                                          **settings)
        ...
        if self.use_domain_adversarial:
            self.logger("sky--- [Domain Adversarial] Adding domain adversarial module at bottleneck layer")
            self.logger(f"sky--- [Domain Adversarial] Bottleneck feature shape: {enc.shape}")
            
            # 梯度反转层
            grl = GradientReversalLayer(lambda_val=self.lambda_val, 
                                        name="{}gradient_reversal".format(name_prefix))(enc)
            
            # 域分类器
            domain_output = self.create_domain_classifier(in_=grl,
                                                          regularizer=regularizer,
                                                          name_prefix=name_prefix)
            
            self.logger(f"sky--- [Domain Adversarial] Domain output shape: {domain_output.shape}")

        """
        Decoding path
        """
        settings["filters"] = filters
        up = self.create_upsample(enc, residual_cons,
                                  attention_module_bottom=self.attention_module_bottom,
                                  **settings)
        ...
        out = self.create_seq_modeling(in_=cls,  # ljy❤: [-1, 23*3840, 5???, 2] -> 每类一个featureMap❤❤❤
                                       ...
                                       activation=activation,
                                       regularizer=regularizer,
                                       attention_module_seq=self.attention_module_seq,
                                       name_prefix=name_prefix)

        # 返回输出：如果使用域对抗，则返回 [sleep_output, domain_output]
        outputs = [out]
        if self.use_domain_adversarial and domain_output is not None:
            outputs.append(domain_output)

        return [inputs], outputs  # ljy❤ out：[-1, 31/23, 3840 // (128*?), 5]
    ...
    def log(self):
        ...
        if self.use_domain_adversarial:
            ...
            self.logger("  Use domain adv:   {}".format(self.use_domain_adversarial))
            self.logger("  N domains:        {}".format(self.n_domains))
            self.logger("  Lambda val:       {}".format(self.lambda_val))
            self.logger("  Domain hidden:    {}".format(self.domain_hidden_units))
