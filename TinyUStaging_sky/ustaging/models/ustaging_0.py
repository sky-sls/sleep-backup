...
class UStaging(Model):
    ...
    def __init__(self,
                 ...
                 batch_shape,
                 depth=4,
                 dilation=(1, 2, 1, 2),  # TODO: ASPP + 避免gridding effect
                 activation="elu",
    ...
    def create_upsample(self,
        ...
        if attention_module_bottom is not None:
            self.logger(f'Attention - Bottom: {attention_module_bottom}')
    ...
    def create_dense_modeling(self,
        ...
        if attention_module_dense is not None:
            self.logger(f'Attention - Dense: {attention_module_dense}')
            in_ = attach_attention_module(in_,
