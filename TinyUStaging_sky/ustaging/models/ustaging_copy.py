...
class UStaging(Model):
    ...
    def __init__(self,
                 ...
                 batch_shape,
                 depth=4,
                 dilation=(1, 2, 3, 4),  # TODO: ASPP + 避免gridding effect
                 activation="elu",
        ...
        # ljy改：
        self.dilation = [dilation] * self.depth if not \
    ...
    def create_dense_modeling(self,
        ...
        self.logger("ljy--- [create_dense_modeling(1*1 Conv)] - cls.get_shape().as_list(): ", cls.get_shape().as_list())
        self.logger("ljy--- [create_dense_modeling(1*1 Conv)] - in_reshaped.get_shape().as_list(): ", in_reshaped.get_shape().as_list())
        return out  # [-1, 31/23*3840, 5？, 2] ？？？

    @staticmethod
    def create_seq_modeling(in_,  # ljy❤: [-1, 31/23*3840, 5？？？, 2] -> 每类一个FeatureMap❤❤❤
                            ...
                            regularizer=None,
                            name_prefix=""):
        cls = AveragePooling2D((data_per_period, 1),  # ljly❤❤ [-1, n_periods(28/31), 5, 2] -> 每类一个FeatureMap❤(转为Label长度)
    ...
    def init_model(self, inputs=None, name_prefix=""):
        ...
        settings = {
            ...
            "activation": activation,
            "dilation": self.dilation,  # TODO: [1,2,3,4...] -> 避免gridding effect
