...
def sequence_predict_generator(model, total_seq_length, generator,
    ...
    for X, _ in generator:
        ...
        if n_batches == 1:
            ...
            print("  batch_pred first row =", batch_pred.reshape(-1, batch_pred.shape[-1])[0])
...
#     print("DEBUG sequence_predict_generator:")
#     print("  total_seq_length =", total_seq_length)
#     print("  output shape template s =", s)
#     print("  pred init shape =", pred.shape)

#     cur_pos = 0
#     n_batches = 0

#     # for X, _, _ in generator: sky0311修改，原本为 for X, _,_ generator:
#     for X, _ in generator:
#         n_batches += 1
#         print("  DEBUG batch {}: X.shape={}, X.dtype={}".format(n_batches, X.shape, X.dtype))
#         print("  DEBUG batch {}: X min/max=({},{}) mean/std=({},{})".format(
#             n_batches, np.min(X), np.max(X), np.mean(X), np.std(X)
#         ))
...
#         batch_pred = model.predict_on_batch(X)
#         print("  DEBUG batch {}: batch_pred.shape={}".format(n_batches, batch_pred.shape))
#         print("  DEBUG batch {}: batch_pred min/max=({},{})".format(
#             n_batches, np.min(batch_pred), np.max(batch_pred)
#         ))
#         print("  DEBUG batch {}: batch_pred first row={}".format(
#             n_batches, batch_pred.reshape(-1, batch_pred.shape[-1])[0]
#         ))
...
#     print("  pred min/max = ({},{})".format(np.min(pred), np.max(pred)))
#     print("  pred nonzero count =", np.count_nonzero(pred))
