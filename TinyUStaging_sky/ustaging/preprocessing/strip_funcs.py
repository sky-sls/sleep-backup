...
from ustaging.errors import NotLoadedError, StripError
...
def strip_to_match(psg, hyp, sample_rate, class_int=None, check_lengths=False, **kwargs):
    ...
    print("ljy-log-20221027: [strip_to_match]", "\npsg.shape: ", psg.shape, "\npsg_length_sec: ", psg_length_sec)
...
def drop_class(psg, hyp, class_int, sample_rate,
    ...
    print("ljy-log-20221027: [drop_class]", "\npsg.shape: ", psg.shape, "\nhyp: ", hyp)
    return psg, hyp
...
def trim_psg_trailing(psg, sample_rate, period_length_sec, hyp=None, **kwargs):
    ...
    i = sample_rate * period_length_sec
    psg1 = psg
    print("ljy-log-20221027: [trim_psg_trailing (before)]", "\npsg.shape: ", psg.shape, "\ni = rate*30s = ", i)

    if len(psg) % i != 0:
        psg1 = psg[:-int(len(psg) % i)]
        if psg1.shape[0] == 0:
            psg1 = psg[0:-int(len(psg) % i), :]
    print("ljy-log-20221027: [trim_psg_trailing3 (after)]", "\npsg1.shape: ", psg1.shape, "\nhyp: ", hyp)
    return psg1, hyp
